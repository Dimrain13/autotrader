...
class EODCloserService:
    ...
    async def close_all_positions(self, reason: str = "End of day"):
        ...
        try:
            ...
            try:
                cancelled = await asyncio.to_thread(alpaca_service.cancel_all_orders)
                if cancelled > 0:
            ...
            for position in positions:
                ...
                try:
                    ...
                    if order:
                        # Prefer the real fill price for P&L when available
                        exit_price = current_price
                        if order.get('filled_avg_price'):
                            exit_price = float(order['filled_avg_price'])

                        logger.info(f"✅ Closed {symbol}: {qty} shares @ ${exit_price:.2f} | P&L: ${unrealized_pl:.2f}")

                        try:
                            trade_hist = get_trade_history()
                            await trade_hist.log_trade({
                                'symbol': symbol,
                                'entry_price': position.get('avg_entry_price', exit_price),
